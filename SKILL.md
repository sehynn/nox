...
**Resume/idempotency check, before anything else.** If this is a fresh invocation of a queue nothing has touched, skip this section. If it might be a re-invocation of a queue an earlier, interrupted session already made progress on (the common case: the earlier session crashed or the terminal closed partway through an overnight run), check each queued issue for prior work before processing it again:

1. **Tracker outcome label** (`in-progress`, `nox-blocked`, `nox-route-to-planning`, or the issue is already closed): skip this issue entirely -- it already reached a real, recorded outcome.
2. **Combined remote-state check**, once per run for the whole batch, not once per issue:
   ```bash
   gh pr list --repo {repo} --state all --json number,headRefName,state,isDraft --limit 300
   git ls-remote --heads origin
   ```
   For each issue with no label from step 1, check both fetched lists for a match: the issue-key appears as a segment bounded on both sides by `/` or start/end-of-string in the branch name (`(^|/){issue-key}(/|$)`, case-insensitive, issue-key escaped as a literal before use -- this rule is deliberately symmetric so it catches the key as a leading, middle, or trailing segment, or the entire branch name, uniformly; it assumes `/` as the delimiter, matching this skill's own `{type}/{issue-key}/{slug}` convention -- a team using a different delimiter needs to adapt the boundary character).
3. If a match is found:
   - **A matching PR with state `MERGED`**: the work actually shipped. Treat as done (equivalent to having the label), and note in the Step 6 report that the label write apparently failed on the prior run and should be applied retroactively for tracker hygiene.
   - **Anything else** (an open PR, a closed-but-not-merged PR, or a bare pushed branch with no PR at all): **do not auto-reprocess and do not silently skip.** This is genuinely ambiguous -- it could be an interrupted prior run, or it could be a deliberate human decision (e.g. a PR closed because the approach was wrong) that never got labeled before something interrupted the label write. Flag it: "{issue-key} has {what was found} with no tracker label -- possible interrupted prior run, or an unlabeled human decision. Not auto-reprocessing. Apply the correct label if there's a real outcome to record, or delete the remote branch (`git push origin --delete {branch}`) to clear this and allow a fresh run." Multiple matches for the same issue: flag all of them together, don't guess which is authoritative.
4. Only if no label and no match in either fetched list is the issue genuinely untouched -- process it fully from scratch.

Accepted, disclosed limitation: a crash in the literal instant of a `git push` or `gh pr create` call, or GitHub API eventual-consistency delays immediately after one, aren't guaranteed to be caught by this check -- narrow, low-probability windows, named outright rather than engineered around. A branch whose slug portion itself contains slash-delimited text that happens to match a different real issue-key's boundary pattern could also false-positive-match -- contrived enough (requires literal slashes inside a slug) to disclose rather than chase further.
...
If a local branch matching this issue's expected name already exists (e.g. left over from an interrupted prior attempt that never got pushed), delete it and start fresh rather than reusing it -- this is a secondary safety net; the primary resume check already happened in Step 1 against remote state, which is what actually matters if the local environment doesn't survive a crash.
...
**Implementation attempt cap, so a stuck loop doesn't run forever.** The design-review loop (3.4) already has a hard cap (10 rounds) for exactly this reason; implementation had no equivalent. Count attempts, not clock time -- each time 3.6 fails and 3.5 gets re-attempted is one attempt, using the same increment-a-counter mechanism 3.4 already uses for review rounds, no timestamps involved:

```
max_attempts = max(3, min(8, 3 + files_touched_in_design))
```

Floor of 3: even a trivial one-file issue gets real room to recover from a fixable mistake before giving up. Ceiling of 8: this skill's own preconditions already gate what reaches implementation at all (small issues, 4-8 per batch) -- 3.4 has no fixed file-count threshold of its own (it's a judgment call, not a number), but a design that reached implementation with 5+ declared files is already in the territory that judgment call exists to catch before implementation starts. Giving it more than 8 attempts wouldn't fix an issue that's actually too big; it would just extend a stuck loop longer.

This is a retry-ceiling nudge, not a prediction of how long or how many attempts implementation "should" take -- `files_touched` is a loose, known-imperfect proxy (a single gnarly file can need more attempts than five mechanical renames), used only to widen or narrow the ceiling a little, not to estimate effort precisely.

**On hitting the cap**: mark the issue **BLOCKED** (reason: "implementation failed `dod` after {N} attempts, still incomplete -- see the last attempt's failure for what's actually going wrong"). Don't read the cap being hit as evidence 3.4 should have called `TOO_LARGE` -- that's the same signal restated, not independent confirmation. Clean up the incomplete branch per the failure handling above.
...
| v5 | Added a resume/idempotency check to Step 1: a crashed or interrupted overnight session previously had no way to be safely re-invoked -- issues that already finished would get silently reprocessed (duplicate branches/PRs) and issues with a real BLOCKED/TOO_LARGE determination would get re-judged from scratch. Eleven design-review rounds went into the matching logic alone, each one a genuinely different bug, not repetition: checking only the tracker label missed the crash-after-push-before-label window; checking `gh pr list` alone missed bare pushed branches; checking `git ls-remote` alone missed merged PRs with auto-deleted branches; treating CLOSED-not-merged PRs as safe-to-redo ignored that a human might have closed one deliberately; prefix-matching on `{type}/{issue-key}/` assumed `{type}` was deterministic across runs, which was never established; raw substring matching on the issue-key alone false-matched `MUN-1` inside `MUN-100`; two rounds of enumerated boundary forms each missed a mirror-image position (trailing slug omitted, then leading type omitted) before landing on one symmetric boundary rule that covers all positions by construction instead of by enumeration. |
| v6 | Added an implementation attempt cap to 3.6 -- the design-review loop (3.4) already had a hard round cap so it couldn't loop forever; implementation had no equivalent, so a stuck `dod`-fails-then-retries loop could run indefinitely. First round proposed a wall-clock minute budget scaled by declared file count and FAILED: file count is a weak proxy for effort, the "hitting the ceiling proves 3.4 should have called TOO_LARGE" framing was circular (the same signal restated, not independent evidence), and no self-monitoring protocol was specified for how an agent would actually track elapsed minutes. Second round switched to counting attempts instead of minutes (the same mechanism 3.4 already uses for review rounds -- no clock needed), reframed the file-count scaling as a loose retry-ceiling nudge rather than an effort estimate, and dropped the circular TOO_LARGE claim entirely. Passed twice independently. |
